...
func (t *gcsFuseCSILustreDualVolumeTestSuite) DefineTests(driver storageframework.TestDriver, pattern storageframework.TestPattern) {
	...
	ginkgo.It("should preserve or correctly remap ownership and permissions when copying files between the GCS Fuse and Lustre mounts", func() {
		...
		tPod.VerifyExecInPodSucceed(f, specs.TesterContainerName,
			fmt.Sprintf("echo 'from-lustre' > %v/from-lustre.txt && chmod 644 %v/from-lustre.txt", lustreMountPath, lustreMountPath))

		ginkgo.By("Verifying the Lustre file reports the real process owner uid (0/root)")
		tPod.VerifyExecInPodSucceed(f, specs.TesterContainerName,
			fmt.Sprintf("test $(stat -c%%u %v/from-lustre.txt) -eq 0", lustreMountPath))

		ginkgo.By("Copying the Lustre-owned file onto the GCS Fuse mount")
		tPod.VerifyExecInPodSucceed(f, specs.TesterContainerName,
			fmt.Sprintf("cp %v/from-lustre.txt %v/from-lustre.txt", lustreMountPath, gcsFuseMountPath))
		...
		tPod.VerifyExecInPodSucceed(f, specs.TesterContainerName,
			fmt.Sprintf("test $(stat -c%%u %v/from-lustre.txt) -eq %d", gcsFuseMountPath, gcsFuseMappedUID))

		ginkgo.By("Writing a file directly on the GCS Fuse mount")
		tPod.VerifyExecInPodSucceed(f, specs.TesterContainerName,
			fmt.Sprintf("echo 'from-gcs' > %v/from-gcs.txt", gcsFuseMountPath))

		ginkgo.By(fmt.Sprintf("Verifying the GCS Fuse file also reports the mounted uid (%d)", gcsFuseMappedUID))
		tPod.VerifyExecInPodSucceed(f, specs.TesterContainerName,
			fmt.Sprintf("test $(stat -c%%u %v/from-gcs.txt) -eq %d", gcsFuseMountPath, gcsFuseMappedUID))

		ginkgo.By("Copying the GCS Fuse file onto the Lustre mount")
		tPod.VerifyExecInPodSucceed(f, specs.TesterContainerName,
			fmt.Sprintf("cp %v/from-gcs.txt %v/from-gcs.txt", gcsFuseMountPath, lustreMountPath))

		ginkgo.By("Verifying Lustre preserves the real process-owner uid (0/root) for the copy, not the GCS Fuse mapped uid")
		tPod.VerifyExecInPodSucceed(f, specs.TesterContainerName,
			fmt.Sprintf("test $(stat -c%%u %v/from-gcs.txt) -eq 0", lustreMountPath))
